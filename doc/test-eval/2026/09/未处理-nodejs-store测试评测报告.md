...
- **一句话结论**：功能主链路、四后端真实库、契约对拍、覆盖率门禁与重复执行稳定性均达标，工程化程度为三仓最高；但被**共享核心的两个 Critical 缺陷**（GQL 关系带参+子选择集解析失败、`$where` 载荷致条件静默丢失）拖累，只能判为「合格」。
- **最严重问题（≤3 条）**：
...
| 3 | 组合与等价类 | 10 | 0.700 | 0 | **7.00** | 状态迁移/类型组合/异常叠加/组合剪裁说明仅部分覆盖 |
| 4 | 交互与集成 | 12 | 0.875 | 2.0 | **8.50** | D-03（M，−2，绑定序列化边界）；I9 并发隔离未通过；I6/I7 部分覆盖 |
| 5 | 压力与性能 | 12 | 0.792 | 0.1 | **9.40** | 无 p99、无分段稳定性观测、无性能门禁（S10=0）；退化 12.1%（I，−0.1） |
...
> 等级约束核对：本仓存在 2 条 Critical（维度 1、7）→ 按 §6 等级映射，「B 良好」要求 `C ≤ 1` 不成立，故加权分上限被压在 C 区间，与本轮 67.7 自洽。
...
| S5 | 吞吐基线对照 | 1 | 1.0 | 1.0 | 历史基线 **637.70 QPS** → 本轮 **560.62 QPS（−12.1%）**；同轮对标 py-store 307.84 / rust-core 279.57 | 对照表见下 |
...
| 负载 | 8×200 | 556.79 → 空闲机复跑 **560.62** | 0 | 13 | 33 | 183 |
| 历史基线 | 8×200 | **637.70**（来源：`doc/2026-09-12-压测报告.md:36` @ 2026-09-12，**本轮未复跑**） | — | — | — | — |

- **R** = 9.5 / 12 = **0.792**；**P** = 0.1（D-09，I 级）；**维度分** = 9.5 − 0.1 = **9.40**
- 本维度缺陷：D-09（I）
...
| D-09 | I | 5 | 吞吐较历史基线下降 12.1%（未达 C 判据） | `node scripts/stress.js`（8×200） | `Ejs-stress-8x200-rerun.log`：`qps=560.62, errors=0`；基线 637.70（`doc/2026-09-12-压测报告.md:36` @ 2026-09-12，本轮未复跑） | DORA 性能回归 | 性能 | 建立复跑脚本与性能门禁，避免回归无人发现 |

**严重度统计**：B **0** / C **2** / M **2** / m **3** / I **1**
...
| 5 压力 | `scripts/stress.js` | 有（2 档） | 有 | 无门禁、无分段/浸泡、无 p99 |
...
| P2 | 增加性能门禁（QPS/尾延迟阈值断言） | S10/D-09 | 回归可自动拦截 | `ci.yml` 增 `stress` job |
